...
class AuctionActor extends Actor with ActorLogging {
  ...
  def inquire(item: Item, bidderOpt: Option[Bidder] = None): AuctionReply = {
    ...
    val reply = auctionOpt match {
      case Some(auction) if bidderOpt != None && auction.highestBidder != None && auction.highestBidder.get == bidderOpt
        .get =>
        auction.status match {
          case Succeeded => Winner(auction.copy())
          case Running => BestOffer(auction.copy())
          case _ => Status(auction.copy(highestBid = None, highestBidder = None))
        }
      case Some(auction) =>
        auction.status match {
          case Succeeded => Status(auction.copy())
          case _ => Status(auction.copy(highestBid = None, highestBidder = None))
        }
      case _ => NotFound
    }
  ...
  def startAuction(item: Item, auctioneer: Auctioneer): AuctionReply = {
    ...
    val reply = auctionOpt match {
      case Some(auction) if auction.auctioneer == auctioneer && auction.status == NotStarted =>
        auctions += (auction.item.name -> auction.copy(status = Running, startedAt = Option(DateTime.now())))
  ...
  def callAuction(item: Item, auctioneer: Auctioneer): AuctionReply = {
    ...
    val auctionOpt = auctions.get(item.name)
    val reply = auctionOpt match {
      case Some(auction) if auction.auctioneer == auctioneer && auction.status == Running =>
        val finalStatus = if (auction.highestBid != None && auction.highestBid.get > item.reservedPrice) Succeeded
        else Failed
        auctions += (auction.item.name -> auction.copy(status = finalStatus, closedAt = Option(DateTime.now())))
        finalStatus match {
          case Succeeded => Status(auctions.get(item.name).get.copy())
  ...
  def offerBid(item: Item, bid: Double, bidder: Bidder): AuctionReply = {
    ...
    val reply = auctionOpt match {
      case Some(auction) =>
        auction.status match {
          case Running if auction.highestBid != None && bid > auction.highestBid.get =>
            auctions += (auction.item.name -> auction.copy(highestBid = Option(bid), highestBidder = Option(bidder)))
            BestOffer(auction.copy())
          case Running if bid > AuctionConfig.MIN_BID_VALUE =>
            auctions += (auction.item.name -> auction.copy(highestBid = Option(bid), highestBidder = Option(bidder)))
            BestOffer(auction.copy())
